...
export const signupUser = async (
  ...
  await supabaseClient.auth.signUp(
    { email, password },
    { data: { name: body.firstName + " " + body.lastName } }
  );
...
export const resetPassword = async (email: string) => {
  const res = await supabaseClient.auth.api.resetPasswordForEmail(email, {
    redirectTo: "/login",
  });
};
...
export const getExerciseData = (
  ...
  let data;
  let graphData = [];
  let currentExercise = personalBestRecords.find(
  ...
  for (let i = 0; i < labels.length; i++) {
    ...
    if (target) {
      graphData.push(
        currentExercise?.weights[currentExercise?.days.indexOf(i)]
      );
  ...
  for (let i = 0; i < exerciseNames.length; i++) {
    ...
    for (let k = 0; k < userDays.length; k++) {
      ...
      if (exerciseRecord.length > 0) {
        let target = personalBestRecords.find(
          (item) => item.name === exerciseNames[i].name
        );
        if (!target) {
          personalBestRecords.push({
            ...
            days: [
              exerciseDay[exerciseRecord.indexOf(Math.max(...exerciseRecord))],
            ],
          });
        } else {
